...
#### WIP: CentOS7 撤退のプロジェクトによる Ubunntu 移行 【 Terraform / Puppet 】

##### 担当業務

- CentOS7 の EOL 対応として、Ubuntu VM に移行
- Puppet、Serverspec

##### 取り組み

- CentOS7 VM は OpenStack の Terraform Provider を用いて count を利用して作成されていたが、Ubuntu VM の作成においては for_each を採用することで、個々のリソースをより柔軟かつ細かく管理可能な構成に変更。
- VM ごとにリクエストの weight を個別に調整できるようになり、負荷分散の設定をより細かく最適化可能に。

#### WIP: AWS インフラコスト削減 【 AWS / Go / GitHub Actions 】
...
- S3 のライフサイクルルールを追加し、CloudWatch Logs の低頻度アクセスストレージクラスへの変更を実施。タグ付けを徹底し、コスト増加要因を迅速に特定。
- 既存の AWS リソースで放置されていた S3 や CloudWatch Logs に関して、社内のログガイドラインに基づき、一定期間経過したリソースをスクリプトで削除。
- 作業をすべて Terraform Import を用いて IaC 化し、リソース管理を効率化。
- S3 と CloudWatch Logs のコストを月に 20% 削減。
...
- スモールスタートとして Lambda から PoC を実施し、最終的には Step Functions、EventBridge Scheduler、ECS on Fargate、CloudWatch Alarms を組み合わせて、コスト削減を実現。
- GitHub Actions を活用して、Go プログラムのリリースバージョンと同じタグを ECR イメージに自動で付与する仕組みを構築し、CI/CD パイプラインを整備。
- Primary DB と Replica DB の監査ログ取得において、Replica 数の変動に柔軟に対応できる仕組みを Terraform で構築。変数を使って簡単に管理できるようにした。
...
- メールヘッダーを解析し、意図した認証を正確に通すことで、Gmail ガイドラインの要件に準拠。他のサービスに対しも、同様の対応を展開。
